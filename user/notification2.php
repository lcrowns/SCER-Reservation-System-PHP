  		...
  		<?php
			...
			<div class = "panel-body">
				...
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$query = $conn->query("SELECT * FROM `transaction` NATURAL JOIN `users` NATURAL JOIN `court` WHERE `status` = 'Cancelled' && `isReadCancelClient` ='No' && `IDnumber`='$id1' && `cancelBy`='Admin'") or die(mysqli_error());
							while($fetch = $query->fetch_array()){
						?>
						<tr>
							
							<td><?php echo $fetch['clientnotifcancel']?></td>

							<td><a class = "btn btn-secondary" href = "readFormCancelClient.php?transactionID=<?php echo $fetch['transactionID']?>" onclick = "confirmationCheckin(); return false;"><i class = "glyphicon glyphicon-check"></i> View</a></td>
						</tr>
						<?php
							}
						?>
