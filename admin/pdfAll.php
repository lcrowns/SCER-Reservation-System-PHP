...
$query = "SELECT * FROM `transaction` NATURAL JOIN `users` NATURAL JOIN `court` WHERE `IDnumber`= '$sd'";
...
$pdf->Cell(59,5, "List of All Transactions",0,1); //EOL
...
while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
	...
	$pdf->SetFont("Arial","B",12);
	$pdf->Cell(40,5,'Feedback:',1,0, '', TRUE);
	$pdf->SetFont("Arial", "",12);
	$pdf->Cell(140,5,"{$feedback}",1,0);
