...
$query = "SELECT * FROM `transaction` NATURAL JOIN `users` NATURAL JOIN `court` WHERE `IDnumber`= '$sd' AND `status` = 'Cancelled'";
...
$pdf->Cell(59,5, "List of Cancelled Transactions",0,1); //EOL
...
while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
	...
	$pdf->SetFont("Arial","B",12);
	$pdf->Cell(60,5,'Reason for Cancellation:',1,0, '', TRUE);
	$pdf->SetFont("Arial", "",12);
	$pdf->Cell(120,5,"{$cancelReason}",1,0);
